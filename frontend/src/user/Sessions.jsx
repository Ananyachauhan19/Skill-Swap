...
const Sessions = () => {
  ...
  const DEMO_SESSIONS = [
    ...
    },
    {
      ...
      description: 'Explore the core concepts of calculus with focus on limits.',
      date: '2025-07-12',
      time: '2:00 PM',
      status: 'Scheduled',
      skillmate: 180,
    ...
    },
    {
      ...
      description: 'Understand motion in one dimension with practical examples.',
      date: '2025-07-15',
      time: '1:00 PM',
      status: 'Scheduled',
      skillmate: 120,
  ...
  const DEMO_UPLOADED_VIDEOS = [
    {
      ...
      unit: 'Algebra',
      topic: 'Linear Equations',
      description: 'Comprehensive guide to algebra basics and linear equations.',
    ...
    return allItems
      ...
      .sort((a, b) => {
        if (a.type === 'Uploaded' && b.type === 'Uploaded') {
          ...
          return (b.skillmate + b.views + b.likes) - (a.skillmate + a.views + a.likes); // Sort live streams by skillmate + views + likes
        } else {
  ...
  const suggestions = [
    ...
    .flatMap(item => [item.course, item.unit, item.topic, item.tutor, item.description])
    .filter((value, index, self) => self.indexOf(value) === index && value.toLowerCase().includes(searchQuery.toLowerCase()))
    .slice(0, 5);
          ...
          {filteredResults().length === 0 ? (
            ...
            <>
              <section className="mb-10">
                ...
                {filteredResults().filter(item => item.type === 'Live').length === 0 ? (
                  <div className="text-gray-500">No live streams found.</div>
                ) : (
                  <LiveStream liveStreams={filteredResults().filter(item => item.type === 'Live')} />
                )}
              </section>
              <section className="mb-10">
                <h2 className="text-xl sm:text-2xl font-bold text-blue-900 mb-4">Scheduled Sessions</h2>
                {filteredResults().filter(item => item.type === 'Scheduled').length === 0 ? (
                  <div className="text-gray-500">No scheduled sessions found.</div>
                ) : (
                  <ScheduledSessions sessions={filteredResults().filter(item => item.type === 'Scheduled')} />
                )}
              </section>
              <section className="mb-10">
                <h2 className="text-xl sm:text-2xl font-bold text-blue-900 mb-4">Uploaded Videos</h2>
                {filteredResults().filter(item => item.type === 'Uploaded').length === 0 ? (
                  <div className="text-gray-500">No uploaded videos found.</div>
                ) : (
                  <UploadedVideos videos={filteredResults().filter(item => item.type === 'Uploaded')} />
                )}
              </section>
            </>
          )}
