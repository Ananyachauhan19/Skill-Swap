...
const Sidebar = () => {
  ...
  return (
    <div className="flex min-h-screen w-full bg-[#f5f8ff] font-sans">
      <aside className="fixed bottom-0 left-0 w-full sm:w-16 sm:static flex sm:flex-col justify-around sm:justify-start items-center gap-1 sm:gap-2 py-2 sm:py-4 sm:min-h-[calc(100vh-4rem)] sm:max-h-[calc(100vh-4rem)] overflow-y-auto scrollbar-hidden bg-transparent z-10 sm:z-auto">
        {[
      ...
      <main className="w-full sm:ml-16 sm:w-[calc(100%-4rem)] min-h-screen p-4 sm:p-6 overflow-y-auto scrollbar-hidden">
        ...
        <div className="h-[calc(100vh-16rem)] sm:h-[calc(100vh-20rem)] overflow-y-auto scrollbar-hidden">
          <Outlet />
        </div>
      </main>
...
></div>
          ...
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[90%] sm:w-[45%] h-[60%] bg-[#f5f8ff] rounded-lg p-6 overflow-y-auto scrollbar-hidden z-30 animate-[fadeIn_0.3s_ease-in]">
              ...
              {(user?.socialLinks || []).length > 0 ? (
                <ul className="flex flex-wrap gap-2">
                  {user.socialLinks.map((link, i) => (
                    <li key={i}>
                      <a href={link.url} className="text-blue-900 text-sm hover:text-blue-700" target="_blank" rel="noopener noreferrer">
                        {link.platform || 'Link'}
                      </a>
                    </li>
                  ))}
