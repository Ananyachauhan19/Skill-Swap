...
const CampusStartSkillSwapSearchForm = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    // Listen for campus tutors response
    socket.on('campus-tutors-found', (data) => {
      console.log('[Campus Search] Received tutors:', data);
      setLoading(false);
      if (data.error) {
        setError(data.error);
        setTutors([]);
    ...
    return () => {
      socket.off('campus-tutors-found');
  ...
  const handleFindTutor = () => {
    ...
    if (!user?.instituteId) {
      setError("You must be part of a campus to use this feature");
      return;
    }
    ...
    console.log('[Campus Search] Searching for tutors with instituteId:', user.instituteId);
    
    // Emit campus-specific search event
    socket.emit('find-campus-tutors', {
      subject: courseValue,
      topic: unitValue,
      subtopic: topicValue,
      question: questionValue,
      questionPhoto: questionPhoto ? questionPhoto.name : null,
      instituteId: user.instituteId, // Pass institute filter
    });
  };
  ...
  const handleRequestSession = async (tutor) => {
    try {
      ...
      socket.emit('send-session-request', {
        ...
        questionPhoto: questionPhoto ? questionPhoto.name : null,
        message: `[Campus] I would like to learn ${unitValue} - ${topicValue}${questionValue ? `: ${questionValue}` : ''}`,
      });
    } catch {
        ...
        >
          {loading ? (
            <span className="flex items-center justify-center gap-2">
              ...
              Finding Campus Tutors...
            </span>
          ) : 'Find Campus Tutor'}
        </button>

        {error && (
          <div className="mt-6 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm flex items-start gap-2">
        ...
        {loading && (
          <div className="mt-8 text-center py-8">
            <div className="flex items-center justify-center gap-3">
              ...
              <span className="text-slate-700 font-medium">Finding campus tutors...</span>
            </div>
          </div>
        )}
...
export default CampusStartSkillSwapSearchForm;
