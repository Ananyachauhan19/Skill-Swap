...
const JoiningCertificateVerify = () => {
  ...
  const fetchCertificate = async () => {
    try {
      ...
      const res = await axios.get(
        `${BACKEND_URL}/api/public/joiningcertificate/${internEmployeeId}`
      );
        ...
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center">
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <p className="text-sm text-red-700">
              The certificate you're trying to verify could not be found or may have been revoked.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 flex flex-col items-center justify-center p-4">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600 text-sm">This is an authentic joining letter issued by SkillSwap Hub</p>
      </div>

      {/* Download Button */}
      <div className="mb-4">
        <a
          href={`${BACKEND_URL}/api/public/joiningcertificate/${internEmployeeId}/download`}
          download
          className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-lg font-medium"
        >
          <FiDownload size={20} />
...
export default JoiningCertificateVerify;
