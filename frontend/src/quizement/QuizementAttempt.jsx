...
const QuizementAttempt = () => {
  ...
  const startTest = async () => {
    try {
      ...
      const initialMarked = {};
      response.data.test.questions.forEach((_, index) => {
      ...
      // Set timer
      setTimeRemaining(response.data.test.duration * 60);
  ...
  const logViolation = async (type) => {
    ...
    try {
      const response = await axios.post(
        `${BACKEND_URL}/api/quizement/tests/${testId}/violation`,
        { type },
        { withCredentials: true }
      );
  ...
  const submitTest = async () => {
    ...
    try {
      ...
      await axios.post(
        `${BACKEND_URL}/api/quizement/tests/${testId}/submit`,
  ...
  const subjectLine = (test.description || '').trim();
            ...
            <div className="h-full bg-white border border-blue-100 rounded-2xl shadow-sm p-4 sm:p-5 flex flex-col overflow-hidden">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-600 truncate">User</p>
                </div>
              </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-blue-100/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl border border-blue-100 shadow-sm max-w-md w-full p-6">
            <h3 className="text-lg font-extrabold mb-3 text-slate-900">Submit Test?</h3>
...
export default QuizementAttempt;
