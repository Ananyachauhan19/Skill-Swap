...
exports.createReport = async (req, res) => {
  try {
    ...
    try {
      const subject = `New ${type === 'video' ? 'Video' : type === 'request' ? 'Request' : 'Account'} Report`;
      const html = `
        <div style="font-family: system-ui, Arial; max-width: 640px; margin:0 auto; padding:16px;">
          <h2>New ${type === 'video' ? 'Video' : type === 'request' ? 'Request' : 'Account'} Report</h2>
          ...
          ${
            type === 'video'
              ...
              : type === 'request'
                ? `<p><b>Request ID:</b> ${doc.requestId || '—'}<br/><b>Request Type:</b> ${doc.requestType || '—'}<br/><b>Status:</b> ${doc.requestStatus || '—'}<br/>${doc.requestCompany ? `<b>Company:</b> ${doc.requestCompany}<br/>` : ''}${doc.requestPosition ? `<b>Position:</b> ${doc.requestPosition}<br/>` : ''}${doc.requestSubject ? `<b>Subject:</b> ${doc.requestSubject}<br/>` : ''}${doc.requestTopic ? `<b>Topic:</b> ${doc.requestTopic}<br/>` : ''}<b>Reported User:</b> ${doc.reportedUsername || '—'} (${doc.reportedUserId || '—'})</p>`
